...
const server = http.createServer((req, res) => {
  ...
  if (pathname === '/overview' || pathname === '/') {
    ...
    const cardsHtml = apiDataObj.map((card) => replaceTemplate(tempCard, card));

    res.end(tempOverview.replace('{%PRODUCT_CARDS%}', cardsHtml.join('')));
  }
  // Product page
  else if (pathname === '/product') {
    res.writeHead(200, { 'Content-type': 'text/html' });
    const product = apiDataObj.filter((el) => el.id == query.id)[0];
